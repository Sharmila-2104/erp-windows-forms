...
{
    public partial class NatureOfGroup : Form
    {
        ...
        public NatureOfGroup()
        {
            InitializeComponent();
            Eve.FormController(this);
        }

        private void NatureOfGroup_Load(object sender, EventArgs e)
        {
            GetData();
            Clear();
        }
        ...
        private void GV_List_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0)
                return;
            ...
            if (GV_List.Columns[e.ColumnIndex].HeaderText == "Delete")
            {
                ...
                if (Confirm == DialogResult.Yes)
                {
                    try
                    {
                        T_DTO.Number = Convert.ToInt64(GV_List.Rows[e.RowIndex].Cells["NumberColumn"].Value);
                        T_DTO.Id = 203;
                        DS = T_DAO.TaxSubMasterDB(T_DTO);
            ...
            else if (GV_List.Columns[e.ColumnIndex].HeaderText == "Edit")
            {
                ...
                if (DS.Tables[0].Rows.Count > 0)
                {
                    ...
                    Txt_Title.Text = DS.Tables[0].Rows[0]["Nature"].ToString();
        ...
        Boolean Duplicate(String Number)
        {
            ...
            {
                T_DTO.Number = Convert.ToInt64(Txt_Number.Text);
                T_DTO.Title = Txt_Title.Text;
                T_DTO.Id = 207;
                DS = T_DAO.TaxSubMasterDB(T_DTO);
            }
